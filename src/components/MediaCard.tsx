...
function getCardWidth(type: MediaItem["type"]): string {
  switch (type) {
    case "movie": case "anime": case "manga": case "music": return "w-52";
    case "tv":    case "web":                               return "w-80";
    default:                                                return "w-52";
  }
}
...
export const MediaCard = memo(function MediaCard({ item, onSelect }: MediaCardProps) {
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 rounded-lg pointer-events-none opacity-0 group-hover:opacity-100 group-focus:opacity-100 transition-opacity duration-200"
        style={{ border: "1px solid rgba(153,247,255,0.25)", borderRadius: "2rem" }} />
    </motion.button>
  );
});

  ...
  return (
    <>
      ...
      <motion.div
        ...
        animate={{ width: cursor.snapped ? 80 : 40, height: cursor.snapped ? 80 : 40, opacity: cursor.snapped ? 1 : 0.5 }}
        transition={{ duration: 0.3, ease: [0.16, 1, 0.3, 1] }}
      />
      {wsConnected && (
        <div className="fixed bottom-8 right-8 z-50 flex items-center gap-2">
          <div className="w-2 h-2 rounded-full bg-primary animate-pulse" />
          <span className="font-mono-tech text-primary text-[10px] opacity-60">AIR MOUSE</span>
        </div>
      )}
    </>
  );
